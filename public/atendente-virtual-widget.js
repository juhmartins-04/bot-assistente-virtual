  ...
  function montar(config) {
    ...
    root.innerHTML = `
      <style>
        ...
        .avw-link-btn { display: inline-flex; align-items: center; justify-content: center; width: 100%; padding: 9px 10px; border-radius: 10px; background: ${cor}; color: #fff; font-weight: 700; font-size: 13px; text-decoration: none; }
        ...
        .avw-lead-continuar { flex: 1; padding: 8px 10px; border-radius: 8px; background: ${cor}; color: #fff; font-size: 13px; font-weight: 700; text-align: center; text-decoration: none; }
    ...
    function pedirContatoEAbrir(label, url, opcaoLabel) {
      ...
      caixa.innerHTML = `
        ...
        <div class="avw-lead-acoes">
          <a class="avw-lead-continuar" target="_blank" rel="noopener noreferrer" href="${seguro || "#"}">${label}</a>
      ...
      function seguirParaWhats() {
        ...
        msg.className = "avw-msg";
        if (seguro) {
          msg.innerHTML = `<a class="avw-link-btn" target="_blank" rel="noopener noreferrer" href="${seguro}">${label}</a>`;
        } else {
          msg.textContent = label;
        }
        body.appendChild(msg);
